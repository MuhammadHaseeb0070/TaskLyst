  ...
  return (
    <TouchableOpacity
      ...
      className={`rounded-xl px-4 py-4 mb-3 border-2 ${
        selected ? "border-[#313d49] bg-[#F8FAFC]" : "border-[#E5E7EB]"
      }`}
    >
      <View className="flex-row items-center justify-between">
        ...
        <View
          className={`w-6 h-6 rounded-full border-2 items-center justify-center ${
            selected ? "border-[#313d49] bg-[#313d49]" : "border-[#D1D5DB]"
          }`}
        >
          {selected && <Text className="text-white text-[12px]">✓</Text>}
        </View>
      </View>
    </TouchableOpacity>
  );
...
export default function CreateServiceStep3Screen() {
  const insets = useSafeAreaInsets();
  ...
  const handleSave = () => {
    ...
    router.push("/create-service-success");
  };
