...
// Settings Icon Component - Reusing original icon from create-service-step2 (NO TINT)
const SettingsIcon = () => (
  <Image
...
// Lightning Icon Component - Reusing original icon from create-service-step2 (NO TINT)
const LightningIcon = () => (
  <Image
...
// Location Icon Component - Reusing original icon from create-service-step2 (NO TINT)
const LocationIcon = () => (
  <Image
...
export default function EditServiceStep2Screen() {
  const insets = useSafeAreaInsets();
  ...
  useEffect(() => {
    if (params.isEdit === "true" && params.serviceId) {
      ...
      if (serviceDetails) {
        // Set duration - convert "30 Mins" format to value
        const durationMatch = serviceDetails.duration.match(/(\d+)/);
        if (durationMatch) {
          const durationValue = durationMatch[1];
          const durationOption = serviceDurationOptions.find(
            (opt) => opt.value === durationValue
          );
          if (durationOption) {
            setServiceDuration(durationOption.value);
            setServiceDurationLabel(durationOption.label);
          }
        }

        // Set special requirements - combine requirements array
        if (serviceDetails.requirements && serviceDetails.requirements.length > 0) {
          setSpecialRequirements(serviceDetails.requirements.join(", "));
        }

        // Set instant booking
        setInstantBooking(serviceDetails.instantBooking);

        // Set route mode - try to match from bookingMode or other fields
        // For now, we'll leave it empty if not found
        const routeModeOption = routeModeOptions.find(
          (opt) => opt.label.toLowerCase() === serviceDetails.bookingMode?.toLowerCase()
        );
        if (routeModeOption) {
          setRouteMode(routeModeOption.value);
          setRouteModeLabel(routeModeOption.label);
        }

        // Set optimize routes - default to true (not stored in ServiceDetails)
